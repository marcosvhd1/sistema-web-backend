...
export default class ServicosController {
  ...
  public async max({ request }: HttpContextContract) {
    ...
    try {
      const max = await Database.from('servicos').max('nserv').where('id_emissor', '=', id_emissor);
      return max;
    } catch (error: any) {
  ...
  public async getAll({ request, response }: HttpContextContract) {
    ...
    try {
      const data = await Database.from('servicos').select('*').whereRaw(`${filter}::TEXT ilike '%${description.toUpperCase()}%'`).andWhere('id_emissor', '=', id_emissor).orderBy('id').paginate(page, limit);
